...
|----------|-------------|
| 需求描述 | 用户记录一笔收入 |
| 业务流程 | 1. 选择「收入」类型<br>2. 输入金额<br>3. 选择类别<br>4. 可选填写备注<br>5. 选择交易日期（默认当天）<br>6. 提交保存 |
| 必填字段 | 金额、类别、日期 |
| 选填字段 | 备注 |
...
|----------|-------------|
| 需求描述 | 用户记录一笔支出 |
| 业务流程 | 1. 选择「支出」类型<br>2. 输入金额<br>3. 选择类别<br>4. 可选填写备注<br>5. 选择交易日期（默认当天）<br>6. 提交保存 |
| 必填字段 | 金额、类别、日期 |
| 选填字段 | 备注 |
...
|------|------|------|
| id | Int | 主键 |
| user_id | Char(12) | 所属用户（null=系统预设） |
| name | Char(50) | 类别名称 |
| type | Int | 1=收入, 2=支出 |
| icon | Char(50) | 图标名称 |
...
|------|------|------|
| id | Int | 主键 |
| user_id | Char(12) | 所属用户（外键） |
| type | Int | 1=收入, 2=支出 |
| amount | Decimal(12,2) | 金额 |
| category_id | Int | 类别 ID（外键） |
| remark | Char(500) | 备注 |
| date | Date | 交易日期（索引） |
| created_at | Datetime | 创建时间 |
| updated_at | Datetime | 更新时间 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/v1/category | 获取类别列表（系统预设+用户自定义） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/v1/report/monthly?year={year}&month={month} | 月度报表 |
| GET | /api/v1/report/yearly?year={year} | 年度报表 |
...
**请求：**
```json
GET /api/v1/transaction?page=1&page_size=20&month=4&year=2025
```
...
**请求：**
```json
GET /api/v1/report/monthly?year=2025&month=4
```
...
**请求：**
```json
GET /api/v1/report/yearly?year=2025
```
