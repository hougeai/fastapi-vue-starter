...
| 需求编号 | FR-AI-001 |
|----------|-----------|
| 需求描述 | 用户通过自然语言描述一笔收支，AI 自动解析为结构化交易记录 |
| 业务流程 | 1. 用户输入自然语言（如"今天午饭花了30元"、"6月工资到账15000"）<br>2. 调用 LLM API 解析为标准 JSON 格式<br>3. 前端展示解析结果供用户确认/修改<br>4. 用户确认后自动存入数据库 |
| 输入模态 | 文本 |
| LLM 输出格式 | `{ "tx_type": 1/2, "amount": number, "category": "类别名", "remark": "备注", "tx_date": "YYYY-MM-DD" }` |
| 异常处理 | LLM 无法解析时返回错误提示，引导用户手动输入 |

#### 2.6.2 语音记账

| 需求编号 | FR-AI-002 |
|----------|-----------|
| 需求描述 | 用户通过语音输入，AI 语音识别转文字后再解析为结构化交易记录 |
| 业务流程 | 1. 用户点击语音输入按钮<br>2. 录制语音<br>3. 调用语音识别 API 转为文字<br>4. 将识别文字按自然语言记账流程处理 |
| 输入模态 | 语音 |
| 依赖 | 语音识别 API（ASR）+ LLM 解析 |
| 交互要求 | 录音时显示波形动画，识别后展示文字供确认 |

#### 2.6.3 图片记账

| 需求编号 | FR-AI-003 |
|----------|-----------|
| 需求描述 | 用户上传小票/账单图片，AI 通过 OCR 或多模态模型识别交易信息 |
| 业务流程 | 1. 用户拍照或上传图片<br>2. 调用多模态 LLM / OCR API 识别图片内容<br>3. 提取交易信息并解析为标准 JSON 格式<br>4. 前端展示解析结果供用户确认/修改<br>5. 用户确认后自动存入数据库 |
| 输入模态 | 图片（JPEG/PNG） |
| 识别内容 | 金额、日期、商户名、类别等 |
| 图片限制 | 单张不超过 10MB，支持 JPEG/PNG/HEIC |
| 异常处理 | 识别失败时提示用户重新拍照或手动输入 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/v1/ai/parse_text | 自然语言解析为交易记录 |
| POST | /api/v1/ai/parse_voice | 语音文件解析为交易记录 |
| POST | /api/v1/ai/parse_image | 图片解析为交易记录 |
...
| AI 记账 | /ai | 多模态输入（文本/语音/图片），AI 解析后确认保存 |
| 我的 | /profile | 用户设置 |
...
- AI 记账：支持文本输入、语音录入、图片上传三种方式，AI 解析后展示预览供确认，确认后自动创建交易记录
...
- [ ] 文本输入自然语言可正确解析为交易记录
- [ ] 语音录入可识别转文字并解析为交易记录
- [ ] 图片上传可识别小票/账单信息并解析为交易记录
- [ ] AI 解析结果展示预览，用户可修改后确认
- [ ] 确认后自动创建交易记录并存入数据库
- [ ] AI 解析失败时给出明确提示，引导手动输入
...
| v1.2 | 2025-06-02 | 新增 AI 记账模块，支持文本/语音/图片多模态输入 |
